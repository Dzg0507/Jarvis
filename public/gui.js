            ...
            <div class="settings-card">
                ...
                <div class="setting-group">
                    ...
                    <select id="settings-voice-select">
                        ${voices.map(voice => 
                            `<option value="${voice.id}" ${voice.id === selectedVoice ? 'selected' : ''}>${voice.name}</option>`
                        ).join('')}
                    </select>
                </div>
...
function setupSettingsEventListeners() {
    ...
    voiceSelect.onchange = () => {
        ...
        const headerVoiceSelect = document.getElementById('voice-select') as HTMLSelectElement;
        if (headerVoiceSelect) headerVoiceSelect.value = selectedVoice;
    };
    ...
    speedSlider.oninput = () => {
        ...
        const headerSpeedSlider = document.getElementById('speed-slider') as HTMLInputElement;
        const headerSpeedValue = document.getElementById('speed-value')!;
        if (headerSpeedSlider) headerSpeedSlider.value = selectedSpeed.toString();
        if (headerSpeedValue) headerSpeedValue.textContent = `${selectedSpeed.toFixed(1)}x`;
    };
    
    const ttsToggle = settingsScreen.querySelector('#settings-tts-enabled') as HTMLInputElement;
    ttsToggle.onchange = () => {
        ...
        if (ttsButton) ttsButton.classList.toggle('disabled', !isTtsEnabled);
    };
    ...
    // Opacity slider
    const opacitySlider = settingsScreen.querySelector('#chat-opacity') as HTMLInputElement;
...
function resetToDefaults() {
    ...
    if (ttsButton) ttsButton.classList.remove('disabled');
    ...
    if (settingsScreen) {
        const settingsVoiceSelect = settingsScreen.querySelector('#settings-voice-select') as HTMLSelectElement;
        const settingsSpeedSlider = settingsScreen.querySelector('#settings-speed-slider') as HTMLInputElement;
        const settingsSpeedValue = settingsScreen.querySelector('#settings-speed-value');
        const settingsTtsToggle = settingsScreen.querySelector('#settings-tts-enabled') as HTMLInputElement;
        
        if (settingsVoiceSelect) settingsVoiceSelect.value = selectedVoice;
        if (settingsSpeedSlider) settingsSpeedSlider.value = selectedSpeed.toString();
        if (settingsSpeedValue) settingsSpeedValue.textContent = `${selectedSpeed.toFixed(1)}x`;
        if (settingsTtsToggle) settingsTtsToggle.checked = isTtsEnabled;
    }
...
async function handleFormSubmit(e: Event) {
  ...
  if (prompt.toLowerCase() === '/settings') {
    chatInput.value = '';
    console.log('Client-side GUI: Detected /settings command locally. Calling enterSettingsMode().');
    enterSettingsMode();
    return;
  }
...
You can also ask me anything about Multi-Party Computation or request code examples!`;
  ...
  console.log(`Client-side GUI: Sending prompt to server: "${prompt}"`);
  ...
  try {
    ...
    const result = await response.json();
    console.log('Client-side GUI: Received result from server:', result);

    // Check if the response is a tool command
    if (result.response && result.response.tool === 'display_settings') {
      console.log('Client-side GUI: Detected "display_settings" tool command in server response. Calling enterSettingsMode().');
        enterSettingsMode();
    } else {
