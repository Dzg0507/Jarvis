// Assuming these types are available or can be defined
// For now, I'll use `any` as a placeholder.
type Model = any;
type GoogleSearch = (query: string) => Promise<string>;
type ViewTextWebsite = (url: string) => Promise<string>;

interface PaperGeneratorDeps {
    model: Model;
    google_search: GoogleSearch;
    view_text_website: ViewTextWebsite;
}

class PaperGenerator {
    private model: Model;
    private google_search: GoogleSearch;
    private view_text_website: ViewTextWebsite;

    constructor({ model, google_search, view_text_website }: PaperGeneratorDeps) {
        this.model = model;
